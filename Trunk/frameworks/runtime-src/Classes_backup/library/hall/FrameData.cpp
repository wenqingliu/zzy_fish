...
static CFrameData* frameData=NULL;Node* CFrameData::s_sender = NULL;
...
bool CFrameData::SocketDelegateWithRecvData(CSpriteData* pData)
{
	...
	log("CFrameData:SocketDelegateWithRecvData:%p,  MainCmdID:%d, SubCmdID:%d, %p",this,wMainCmdID, wSubCmdID);
...
bool CFrameData::OnSocketSubLogonSuccess(void* pData, WORD wSize)
{
	...
	GetFrameData()->m_dwAccountType = logonSuccess.cbAccountType;
	...
	sendEvent(ccNd_Hall_Sub_MB_LogonResult,CreateNotify(pData,wSize,RT_SUCCESS));
    //通知刷新界面
...
bool CFrameData::OnSocketSubLogonFailure(void* pData, WORD wSize)
{
    ...
    sendEvent(ccNd_Hall_Sub_MB_LogonResult, CreateNotify(failuer, wSize, RT_FAILURE));
    setSocketClose();
    log("CFrameData: 登录失败");
	return true;
}
...
bool CFrameData::OnSocketSubListFinish(void* pData, WORD wSize)
{
    ...
    //处理通知
    sendEvent(ccNd_Hall_Sub_GR_ConfigList, CreateNotify(pData, wSize, RT_ENDED));
...
bool CFrameData::OnSocketSubListServer(void* pData, WORD wSize)
{
    ...
    //配置房间列表
	sendEvent(ccNd_Hall_Sub_GR_ConfigList, CreateNotify(pData, wSize, RT_INGED));
...
//                                   decrypt.c_str());
    ...
    pFrameData->sendData(MDM_MB_LOGON, SUB_MB_LOGON_PTHERPLATFORM, &request, sizeof(request));
}
...
bool CFrameData::OnSocketSubUserEnter(void* pData, WORD wSize)
{
	...
	if(pInfoHead.dwUserID == m_dwUserID)
	{
        ...
        m_wCustom  = pInfoHead.dwCustomID;
        m_wFaceID  = pInfoHead.wFaceID;
