...
void ClineSeg::init(Vec2 start,Vec2 end){
    startPoint=start;
    endPoint=end;
}
...
Vec2 ClineSeg::getDir()const {
    return ccpNormalize(endPoint-startPoint);
}
Vec2 ClineSeg::getVec()const {
    return endPoint-startPoint;
}
Vec2 ClineSeg::getRightNormal()const {//right side normal or outer normal
    Vec2 dir=getDir();
    Vec2 normal(dir.y,-dir.x);
    return normal;
}
Vec2 ClineSeg::getMidPoint()const {
...
bool ClightningBoltSprite::init(const std::string& filename){
...
vector<ClineSeg*> ClightningBoltSprite::splitSeg(ClineSeg*seg,int generationID)
{
   // CCLOG("seg->father branchID:%i",seg->branchID);
    ...
    vector<ClineSeg*> seges;
    {
        ...
        Vec2 midPoint=seg->getMidPoint()+ccpMult(seg->getRightNormal(),offset);
        ...
        if(catchProbability(p))
        {
            ...
            Vec2 splitEnd=ccpMult(rotateVector2(seg0->getVec(),randomSmallAngle), lengthScale)+midPoint;
...
void ClightningBoltSprite::genMesh(){
    ...
    //make mesh
    {
        ...
        for(int i=0;i<nSeg;i++){
            ...
            //       *--------*
            //      v1        v2
            const Vec2&start=seg->startPoint;
            const Vec2&end=seg->endPoint;
            Vec2 rightNorm=seg->getRightNormal();
            float segW=12*nPow(0.3,seg->branchID);
            float opacity=1.0*nPow(0.4, seg->branchID);
            Vec2 p0=end+ccpMult(rightNorm, segW/2);
            Vec2 p1=end+ccpMult(rightNorm, -segW/2);
            Vec2 p2=start+ccpMult(rightNorm, -segW/2);
            Vec2 p3=start+ccpMult(rightNorm, segW/2);
            {
                Vec2 mid=seg->getMidPoint();
            ...
            Vec2 texCoord0=Vec2(0,0);
            Vec2 texCoord1=Vec2(0,1);
            Vec2 texCoord2=Vec2(1,1);
...
void ClightningBoltSprite::setStartAndEnd(Vec2 start,Vec2 end){
    ...
    if(m_headSeg&&m_tailSeg)
    {
        ...
        Vec2 vec_headStartToStart=m_startPoint-m_headSeg->startPoint;
        int nseg=(int)m_segList.size();
        for(int i=0;i<nseg;i++){
            ClineSeg*seg=m_segList[i];
        ...
        Vec2 vec_headStartToTailEnd=m_tailSeg->endPoint-m_headSeg->startPoint;
        Vec2 vec_headStartToEnd=this->m_endPoint-m_headSeg->startPoint;
        Vec2 cosA_sinA=calculateCosASinAOfVec1ToVec2(vec_headStartToTailEnd, vec_headStartToEnd);
...
void ClightningBoltSprite::debugDraw_segWireFrame(float lineWidth){
    ...
    for(int i=0;i<nIDtri;i++){
        ...
        Vec2 vertices[3]={ccp(p0.x(),p0.y()),ccp(p1.x(),p1.y()),ccp(p2.x(),p2.y())};
...
//---------------------------------------------
bool ClightningBoltNode::init(const std::string& filename,const Vec2&start,const Vec2&end){
    m_start=start;
    m_end=end;
    //lightingSprit
    {
...
void ClightningBoltNode::runFlashActionOnce(){
    ...
    CallFunc*flashForever = CallFuncN::create([=](Node*target){
        ...
        CCSequence*seq=CCSequence::create(vector);
        //----make whole action
        CCAction*action=CCSpawn::create(seq,CCSequence::create(resetOpacity,fadeTo,genLightning,NULL),NULL);
        //----runAction
        ((ens::ClightningBoltSprite*)target)->runAction(action);
    });

    CallFunc* onComplete = CallFuncN::create(   [=](Node*target)    {this->removeFromParent();}    );

    CCAction*action1=CCSequence::create(dynamic_cast<CallFunc*>(flashForever->clone()),NULL);
    CCAction*action2=CCSequence::create(delay,dynamic_cast<CallFunc*>(flashForever->clone()),onComplete,NULL);
    this->getLBSprite1()->runAction(action1);
    this->getLBSprite2()->runAction(action2);
}
