...
void CtailSprite::update(float dt){
    //add a pos
    Vec2 posCur=this->getPosition();
    if(m_footPrintList.empty()){
        m_footPrintList.push_back(CfootPrint(posCur,m_initialOpacity));
    }else{
        Vec2 posFoe=m_footPrintList[(int)m_footPrintList.size()-1].m_pos;
...
Cmesh* CtailSprite::generateMesh(const vector<CfootPrint>&footPrintList){
    ...
    for(int i=0;i<nFootPrint-1;i++){
        const Vec2&start=footPrintList[i].m_pos;
        const Vec2&end=footPrintList[i+1].m_pos;
    ...
    for(int i=0;i<nFootPrint;i++){
        if(i==0){
            ...
            Vec2 posf=footPrintList[i-1].m_pos;
            Vec2 pos=footPrintList[i].m_pos;
            Vec2 posn=footPrintList[i+1].m_pos;
            Vec2 vec=pos-posf;
            Vec2 vecn=posn-pos;
    ...
    for(int i=0;i<nFootPrint-1;i++){
        ...
        //
        Vec2 start=footPrintList[i].m_pos;
        Vec2 end=footPrintList[i+1].m_pos;
        Vec2 dir=ccpNormalize(end-start);
        //
        Vec2 normLeft=Vec2(-dir.y,dir.x);
        Vec2 normRight=Vec2(0,0)-normLeft;
        Vec2 LU=start+ccpMult(normLeft, R);
        Vec2 LD=start+ccpMult(normRight, R);
        Vec2 RU=end+ccpMult(normLeft, R);
        Vec2 RD=end+ccpMult(normRight, R);
        //  LU----RU
        //   |               |
        // start--end
        //   |               |
        ...
        upQuad.m_posRU=RU;
        upQuad.m_posRD=end;
        upQuad.m_texCoordLU=Vec2(curLengthFoe/totalLength,0);
        upQuad.m_texCoordLD=Vec2(curLengthFoe/totalLength,0.5);
        upQuad.m_texCoordRU=Vec2(curLength/totalLength,0);
        upQuad.m_texCoordRD=Vec2(curLength/totalLength,0.5);
        ...
        dnQuad.m_posRU=end;
        dnQuad.m_posRD=RD;
        dnQuad.m_texCoordLU=Vec2(curLengthFoe/totalLength,0.5);
        dnQuad.m_texCoordLD=Vec2(curLengthFoe/totalLength,1);
        dnQuad.m_texCoordRU=Vec2(curLength/totalLength,0.5);
        dnQuad.m_texCoordRD=Vec2(curLength/totalLength,1);
    ...
    for(int i=0;i<nQuad-1;i++){
        ...
        Vec2 leftVec=ccpNormalize(upQuad.m_posLD-upQuad.m_posRD);
        Vec2 rightVec=ccpNormalize(upQuadn.m_posRD-upQuadn.m_posLD);
        ...
        if(turnType==eTurnType_left||turnType==eTurnType_straightBack){
            ...
            Vec2 L1=dnQuad.m_posRD-dnQuad.m_posRU;
            Vec2 L2=dnQuadn.m_posLD-dnQuadn.m_posLU;
            vector<Vec2> LList;//L1,...,L2
            LList.push_back(L1);
            const float dA=30;
            float A=0;
            while (1) {
                A+=dA;
                Vec2 L=Vec2(L1.x*cosf(A*M_PI/180)-L1.y*sinf(A*M_PI/180),
            ...
            for(int j=0;j<nQuadMakeUp;j++){
                ...
                quad.m_texCoordLU=Vec2(dnQuad.m_texCoordRU.x,0.5);
                quad.m_texCoordRU=Vec2(dnQuad.m_texCoordRU.x,0.5);
                quad.m_texCoordLD=Vec2(_sa+j*(_sb-_sa)/nQuadMakeUp,1);
                quad.m_texCoordRD=Vec2(_sa+(j+1)*(_sb-_sa)/nQuadMakeUp,1);
        ...
        if(turnType==eTurnType_right||turnType==eTurnType_straightBack){
            ...
            vector<Cquad> quadList_makeup;
            Vec2 L1=upQuad.m_posRU-upQuad.m_posRD;
            Vec2 L2=upQuadn.m_posLU-upQuadn.m_posLD;
            vector<Vec2> LList;//L1,...,L2
            LList.push_back(L1);
            const float dA=-30;
            float A=0;
            ...
            for(int j=0;j<nQuadMakeUp;j++){
                Cquad&quad=quadList_makeup[j];
                quad.m_texCoordLU=Vec2(_sa+j*(_sb-_sa)/nQuadMakeUp,0);
                quad.m_texCoordRU=Vec2(_sa+(j+1)*(_sb-_sa)/nQuadMakeUp,0);
                quad.m_texCoordLD=Vec2(upQuad.m_texCoordRD.x,0.5);
                quad.m_texCoordRD=Vec2(upQuad.m_texCoordRD.x,0.5);
        ...
        if(turnType==eTurnType_left&&isSharp==false){
            Vec2 upMid=ccpMidpoint(upQuad.m_posRU, upQuadn.m_posLU);
            upQuad.m_posRU=upMid;
            upQuadn.m_posLU=upMid;
        }
        
        if(turnType==eTurnType_right&&isSharp==false){
            Vec2 dnMid=ccpMidpoint(dnQuad.m_posRD, dnQuadn.m_posLD);
        ...
        for(int j=0;j<nQuad;j++){
            ...
            if(PND1*PND2<0&&quad.m_isTriangle==false){//self-intersect
                ...
                float opacityP;
                {
                    ...
                    texCoordP=Vec2((quad.m_texCoordLU.x+quad.m_texCoordRU.x)/2,quad.m_texCoordLU.y+0.5*dis_PtoUp/(dis_PtoUp+dis_PtoDn));
