...
void CbreakSprite::draw(Renderer* renderer, const Mat4 &transform, uint32_t flags){
    ...
    if(m_state==eState_well){
        ...
        this->Sprite::draw(renderer, transform, flags);
...
void CbreakSprite::onDrawDebug(const Mat4 &transform, uint32_t flags){
    ...
    for(int i=0;i<nTri;i++){
        ...
        Vec2 vertices[3]={M4TransV2(m,Vec2(tri.m_pos[0].x(),tri.m_pos[0].y())),
            M4TransV2(m,Vec2(tri.m_pos[1].x(),tri.m_pos[1].y())),
            M4TransV2(m,Vec2(tri.m_pos[2].x(),tri.m_pos[2].y()))};
...
int CbreakSprite::getPointInRangeEqualsToTargetPoint(const Vec2&targetPoint,const vector<Vec2>&pointList,int imin,int imax,float eps)
...
void CbreakSprite::doCrack(const Vec2&touchPoint){
    ...
    Vec2 touchPoint_localSpace=CCPointApplyAffineTransform(touchPoint,parentToNodeTransform);
    ...
    while(1){
        ...
        for(int i=0;i<nIDquad;i++){
            ...
            Vec2 posLU=m_pointList[IDLU];
            Vec2 posLD=m_pointList[IDLD];
            Vec2 posRU=m_pointList[IDRU];
            Vec2 posRD=m_pointList[IDRD];
            Vec2 posCenter=ccpMidpoint(posLU, posRD);
            ...
            if(sideLen/disQuadToTouch>k&&sideLen/2>m_gridSideLenMin){//if sideLen/disQuadToTouch is big and sideLen after divide is bigger than minSideLen
                ...
                //posL
                int IDL;
                Vec2 posL=ccpMidpoint(posLU, posLD);
                {
                ...
                //posU
                int IDU;
                Vec2 posU=ccpMidpoint(posLU, posRU);
                {
                    //find first, if not found, add
                ...
                //posR
                int IDR;
                Vec2 posR=ccpMidpoint(posRU, posRD);
                {
                    //find first, if not found, add
                ...
                //posD
                int IDD;
                Vec2 posD=ccpMidpoint(posLD, posRD);
                {
                    //find first, if not found, add
                ...
                //posC
                int IDC;
                Vec2 posC=ccpMidpoint(posLU, posRD);//center
                {
    ...
    //for a point, we must find all IDquad who use it, in order to determine how big the point offset can be
    {
        ...
        for(int i=0;i<nPoint;i++){
            Vec2&point=m_pointList[i];
            const Vec2&offset=pointOffsetList[i];
            point=point+offset;
        }
    ...
    for(int i=0;i<nIDquad;i++){
        ...
        Vec2 posRU=m_pointList[IDRU];
        Vec2 posRD=m_pointList[IDRD];
        Vec2 center=ccpMidpoint(posLU, posRD);
    ...
    //----make each tri smaller
    {
        ...
        for(int i=0;i<nTri;i++){
            ...
            Vec2 p0=v2Toccp(tri.m_pos[0]);
            Vec2 p1=v2Toccp(tri.m_pos[1]);
            Vec2 p2=v2Toccp(tri.m_pos[2]);
            vector<Vec2> points=makeTriangleSmaller(p0, p1, p2, dOffset);
    ...
    //caculate color of each IDline
    {
        ...
        const Vec2 lightDir0=Vec2(cosf(-45*M_PI/180),sinf(-45*M_PI/180));
        const Vec2 lightDir1=Vec2(cosf(-135*M_PI/180),sinf(-135*M_PI/180));
        ...
        for(int i=0;i<nIDline;i++){
            ...
            Vec2 dir=ccpNormalize(v2Toccp(vB)-v2Toccp(vA));
            Vec2 rightNormal=getRightNormal(dir);
...
void CfallOffAction::startWithTarget(Node *pTarget){
