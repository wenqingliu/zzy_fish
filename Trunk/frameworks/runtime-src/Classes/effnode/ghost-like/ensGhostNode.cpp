...
{
    //----create sprite
    Sprite*sprite=Sprite::create(texFileName.c_str());
...
bool CghostSprite::init(Sprite* sprite,float filterR,float fliterSrcScale){
    m_finalSprite=Sprite::create();
...
void CghostSprite::visit(Renderer* renderer, const Mat4 &parentTransform, uint32_t parentFlags)
{
    ...
    //----draw m_biggerRT to m_filterXRT (use m_program_fliterX)
    {
        ...
        Vec2 offset=ccp(m_filterXRT->getSprite()->getContentSize().width/2,m_filterXRT->getSprite()->getContentSize().height/2);
    ...
    {
        Sprite*sp=m_filterYRT->getSprite();
