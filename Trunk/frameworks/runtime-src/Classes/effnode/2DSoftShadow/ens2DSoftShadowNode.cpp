...
bool C2DSoftShadowObj::updateShadow(){
    ...
    Vec2 lightPosLocal=getLightPosLocal();
    m_pointTypeList.clear();
    int nPoint=(int)m_polygon.m_pointList.size();
    m_pointTypeList.resize(nPoint);
    for(int i=0;i<nPoint;i++){
        const Vec2&P=m_polygon.m_pointList[i];
        CPointType&pointType=m_pointTypeList[i];
        vector<Vec2> LRT=calculateTangentPointsOfPointToCircle(lightPosLocal, m_light->getR(), P);
        ...
        for(int j=0;j<nPoint;j++){
            const Vec2&p=m_polygon.m_pointList[j];
            Vec2 Pp=p-P;
            if(j==i){
    ...
    for(int i=0;i<nPointType;i++){
        const Vec2&point=m_polygon.m_pointList[i];
...
void C2DSoftShadowObj::onDrawDebug(const Mat4 &transform, uint32_t flags){
    ...
    if(m_isUpdateShadowSucc){
        ...
        if(isPointEqual(M4TransV2(transform,m_intersectPoint), Vec2(INFINITY,INFINITY), 0)==false)
        {
            ccPointSize(4);
            ccDrawColor4F(0, 0, 1, 1);
...
Vec2 C2DSoftShadowObj::getLightPosLocal(){
    Vec2 lightPosWorld=m_light->convertToWorldSpaceAR(ccp(0,0));
    Vec2 lightPosLocal=this->convertToNodeSpace(lightPosWorld);
    return lightPosLocal;
}
void C2DSoftShadowObj::makeFullWindowRectMesh(){
    m_mesh->clear();
    CCSize winSize=CCDirector::sharedDirector()->getWinSize();
    const Vec2 p0World=Vec2(winSize.width,winSize.height);//RU
    const Vec2 p1World=Vec2(0,winSize.height);//LU
    const Vec2 p2World=Vec2(0,0);//LD
    const Vec2 p3World=Vec2(winSize.width,0);//RD
    const Vec2 p0=convertToNodeSpace(p0World);
    const Vec2 p1=convertToNodeSpace(p1World);
    const Vec2 p2=convertToNodeSpace(p2World);
    const Vec2 p3=convertToNodeSpace(p3World);
    //v0
    Cv2 pos0=ccpTov2(p0);
...
void C2DSoftShadowObj::updateMesh(){
...

    {
        ...
        if(m_rightPenumbraPointID==m_rightUmbraPointID){
            ...
            while(1){
                const Vec2&p=m_polygon.m_pointList[index];
                const Vec2&pf=m_polygon.m_pointList[index-1>=0?index-1:index-1+nPoint];
                Cedge edge;
                edge.m_start=pf;
        ...
        for(int i=0;i<nEdge-1;i++){
            ...
            const Vec2&p0=edge.m_end;
            const Vec2&p1=edgen.m_end;
            const Vec2&p2=edgen.m_start;
    ...
    //----left penumbra mesh
    {
        ...
        if(m_leftPenumbraPointID==m_leftUmbraPointID){
            ...
            while(1){
                const Vec2&p=m_polygon.m_pointList[index];
                const Vec2&pf=m_polygon.m_pointList[(index+1)%nPoint];
                Cedge edge;
                edge.m_start=pf;
                edge.m_end=edge.m_start+ccpMult(ccpNormalize(p-pf), m_shadowLength);
                edgeList.push_back(edge);
        ...
        for(int i=0;i<nEdge-1;i++){
            ...
            const Cedge&edgen=edgeList[i+1];
            const Vec2&p0=edge.m_end;
            const Vec2&p1=edgen.m_start;
            const Vec2&p2=edgen.m_end;
            float bright=brightList[i];
            float brightn=brightList[i+1];
            //v0
            Cv2 pos0=ccpTov2(p0);
    ...
    //----umbra mesh
    {
        ...
        if(isPointEqual(m_intersectPoint, Vec2(INFINITY,INFINITY), 0)){//parallel
            vector<Cedge> edgeList;
            edgeList.push_back(Cedge(m_rightUmbraLine[1],m_rightUmbraLine[2]));
            Vec2 dir=ccpNormalize(m_rightUmbraLine[2]-m_rightUmbraLine[1]);
            int nPoint=(int)m_polygon.m_pointList.size();
            int index=(m_rightUmbraPointID+1)%nPoint;
            assert(m_rightUmbraPointID!=m_leftUmbraPointID);
            while(1){
                if(index==m_leftUmbraPointID)break;
                const Vec2&point=m_polygon.m_pointList[index];
                Cedge edge;
                edge.m_start=point;
            ...
            for(int i=0;i<nEdge-1;i++){
                const Cedge&edge=edgeList[i];
                const Cedge&edgen=edgeList[i+1];
                const Vec2&p0=edge.m_start;//RU
                const Vec2&p1=edge.m_end;//LU
                const Vec2&p2=edgen.m_end;//LD
                const Vec2&p3=edgen.m_start;//RD
                //v0
                Cv2 pos0=ccpTov2(p0);
            ...
            if(ccpDot(m_leftUmbraLine[1]-m_leftUmbraLine[0],m_leftUmbraLine[1]-m_intersectPoint)>0){//intersectP on back side of light
                ...
                for(int i=0;i<nEdge-1;i++){
                    const Cedge&edge=edgeList[i];
                    const Cedge&edgen=edgeList[i+1];
                    const Vec2&p0=edge.m_start;
                    const Vec2&p1=edge.m_end;
                    const Vec2&p2=edgen.m_start;
                    //v0
                    Cv2 pos0=ccpTov2(p0);
                    Cv2 texCoord0=Cv2(1,1);
    ...
    {
        Vec2 intersectOfPenumbraLines=getIntersectPointOfTwoLine(m_leftPenumbraLine[0], ccpNormalize(m_leftPenumbraLine[1]-m_leftPenumbraLine[0]),
        ...
        if(m_leftPenumbraPointID==m_rightPenumbraPointID){
            ...
            while(1){
                const Vec2&p=m_polygon.m_pointList[index];
                const Vec2&pn=m_polygon.m_pointList[(index+1)%nPoint];
                //
                const Vec2&p0=intersectOfPenumbraLines;
                const Vec2&p1=pn;
                const Vec2&p2=p;
                //v0
                Cv2 pos0=ccpTov2(p0);
        ...
        {
            const float dA=45;
            const Vec2 startDir=ccpNormalize(m_leftPenumbraLine[2]-intersectOfPenumbraLines);
            const Vec2 endDir=ccpNormalize(m_rightPenumbraLine[0]-m_rightPenumbraLine[1]);
            Vec2 dir=startDir;
            Vec2 dirfoe;
            while(1){
                dirfoe=dir;
                ...
                const Vec2&p0=intersectOfPenumbraLines;
                const Vec2&p1=intersectOfPenumbraLines+ccpMult(dirfoe, m_shadowLength);
                const Vec2&p2=intersectOfPenumbraLines+ccpMult(dir, m_shadowLength);
                //v0
                Cv2 pos0=ccpTov2(p0);
                Cv2 texCoord0=Cv2(0,0);
                Cv4 color0=Cv4(1,1,0,1);
        ...
        {
            const float dA=45;
            const Vec2 startDir=ccpNormalize(m_leftPenumbraLine[0]-m_leftPenumbraLine[1]);
            const Vec2 endDir=ccpNormalize(m_rightPenumbraLine[2]-m_rightPenumbraLine[1]);//ccpNormalize(m_rightPenumbraLine[0]-m_rightPenumbraLine[1]);//ccpNormalize(m_rightPenumbraLine[2]-intersectOfPenumbraLines);
            Vec2 dir=startDir;
            Vec2 dirfoe;
            while(1){
                dirfoe=dir;
        ...
        {
            const float dA=45;
            const Vec2 startDir=ccpNormalize(m_rightPenumbraLine[0]-m_rightPenumbraLine[1]);
            const Vec2 endDir=ccpNormalize(m_leftPenumbraLine[0]-m_leftPenumbraLine[1]);
            Vec2 dir=startDir;
            Vec2 dirfoe;
            while(1){
                dirfoe=dir;
...
bool C2DSoftShadowRoot::init(){
    ...
    //m_tempSprite
    m_tempSprite=Sprite::createWithTexture(m_shadowRT->getSprite()->getTexture());
