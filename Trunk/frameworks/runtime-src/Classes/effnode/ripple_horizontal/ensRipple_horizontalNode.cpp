...
void Cripple_horizontalNode::init(const string&texFileName){
    ...
    for(int i=0;i<nPoint;i++){
        float x=i*m_dx;
        Vec2 point(x,contentSize.height);
...
//x is in parentSpace
{
    ...
    Vec2 startPoint_local=m_surfacePointList[0];
    Vec2 endPoint_local=m_surfacePointList[(int)m_surfacePointList.size()-1];
    //convert startPoint and endPoint to parent space
    Vec2 startPoint=CCPointApplyAffineTransform(startPoint_local,nodeToParentTransform());
    Vec2 endPoint=CCPointApplyAffineTransform(endPoint_local,nodeToParentTransform());
    //calculate the intersect point of line(X=x) and line(startPoint-endPoint)
    Vec2 leftPoint=(startPoint.x<endPoint.x)?startPoint:endPoint;
    Vec2 rightPoint=(startPoint.x<endPoint.x)?endPoint:startPoint;
    if(x>leftPoint.x&&x<rightPoint.x){//have intersect`
        ...
        Vec2 touchPoint(x,y);//parent space touchPoint
        ...
        Vec2 touchPoint_local=CCPointApplyAffineTransform(touchPoint,parentToNodeTransform());
        ...
        for(int i=0;i<nPoint;i++){
            ...
            Vec2&point=m_surfacePointList[i];
...
void Cripple_horizontalNode::initMesh(){
    ...
    Vec2 startPoint=m_surfacePointList_reduced[0];
    Vec2 endPoint=m_surfacePointList_reduced[(int)m_surfacePointList_reduced.size()-1];
    const float len=endPoint.x-startPoint.x;
    int nPoint=(int)m_surfacePointList_reduced.size();
    for(int i=0;i<nPoint-1;i++){
        const Vec2&p=m_surfacePointList_reduced[i];
        const Vec2&pn=m_surfacePointList_reduced[i+1];
        //LU
...
void Cripple_horizontalNode::updateRipple(){
    ...
    //update surfacePointList's point height
    {
        ...
        //    if(i==nPoint-1)continue;
            Vec2&point=m_surfacePointList[i];
            Vec2&pointf=m_surfacePointList[i==0?nPoint-1:i-1];
            Vec2&pointn=m_surfacePointList[i==nPoint-1?0:i+1];
            Vec2&point_back=m_surfacePointList_back[i];
        ...
        {
            int nPoint=(int)m_surfacePointList.size();
            Vec2 lastPoint=m_surfacePointList[nPoint-1];
            Vec2 lastPoint_back=m_surfacePointList_back[nPoint-1];
        ...
        vector<Vec2> temp=m_surfacePointList;
...
void Cripple_horizontalNode::onDrawDebug(const Mat4 &transform, uint32_t flags){
    ...
    for(int i=0;i<nSeg;i++){
        Vec2 point=m_surfacePointList_reduced[i];
        Vec2 pointn=m_surfacePointList_reduced[i+1];
