...
void CtinyWingsTerrainSprite::reGenerateShape(int insertControlPointCount){
    ...
    Vec2 startPoint=pointList[0];
    Vec2 endPoint=pointList[(int)pointList.size()-1];
    float len=endPoint.x-startPoint.x;
    float stepLen=len/(insertControlPointCount+1);
    vector<Vec2> controlPointList;
    controlPointList.push_back(Vec2(startPoint.x,m_height));//start control point
    for(int i=0;i<insertControlPointCount;i++){
        float x=(i+1)*stepLen;
        float y=m_height;
        controlPointList.push_back(Vec2(x,y));
    }
    controlPointList.push_back(Vec2(endPoint.x,m_height));//end control point. now got controlPointList
    //random y for each control point
    int nControlPoint=(int)controlPointList.size();
    for(int i=0;i<nControlPoint;i++){
        Vec2&controlPoint=controlPointList[i];
    ...
    for(int i=0;i<nPoint;i++){
        ...
        if(controlPointIndex>=(int)controlPointList.size()){//point is endPoint
            ...
            Vec2 controlPoint=controlPointList[controlPointIndex-1];
            Vec2 controlPointn=controlPointList[controlPointIndex];
...
void CtinyWingsTerrainSprite::updateMesh(){
    ...
    m_mesh->clear();
    Vec2 startPoint=m_pointMat[0][0];
    Vec2 endPoint=m_pointMat[0][(int)m_pointMat[0].size()-1];
    const float len=endPoint.x-startPoint.x;
    int nRow=(int)m_pointMat.size();
    for(int i=0;i<nRow-1;i++){
        int nPoint=(int)m_pointMat[i].size();
        for(int j=0;j<nPoint-1;j++){
            const Vec2&pLU=m_pointMat[i][j];
            const Vec2&pLD=m_pointMat[i+1][j];
            const Vec2&pRU=m_pointMat[i][j+1];
            const Vec2&pRD=m_pointMat[i+1][j+1];
