...
struct tagGameKind
{
	...
	TCHAR							szDescribe[MAX_DESCRIBE][LEN_DESCRIBE]; //区域描述
};
...
struct tagGameServer
{
	...
	TCHAR							szServerAddr[32];					//房间名称（应该是服务器地址吧）
	TCHAR							szServerName[LEN_SERVER];			//房间名称
};
//视频配置
...
struct tagUserProperty
{
	...
	struct tagUsePropertyInfo              PropertyInfo[MAX_PT_MARK];			//使用信息   
};
...
struct tagUserInfo
{
	...
	//时间信息
	struct tagTimeInfo						TimerInfo;							//时间信息
...
struct tagMobileUserInfoHead
{
	...
	//新增
	TCHAR							szNickName[LEN_NICKNAME];			//用户昵称
	TCHAR 							szFaceIDDownURL[LEN_USER_NOTE];		//自定义头像地址
};
//////////////////////////////////////////////////////////////////////////////////
